...
def dispatch_jobs():
    ...
    # Dispatch only the ones for which there are no reports yet (to avoid avoerwriting)
...
def compile_reports():
    ...
    cves_to_analyse = set(config.cves) & set(
        [fn.replace(".json", "") for fn in os.listdir(config.prospector_reports)]
    )  # get the set of reports where a CVE is set in config.yaml and the corresponding report got successfully generated
    print(f"Analysed {len(cves_to_analyse)} CVEs: {cves_to_analyse}")
